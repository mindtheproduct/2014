<?php
/**
 * The template for displaying all pages.
 *
 * This is the template that displays all pages by default.
 * Please note that this is the WordPress construct of pages
 * and that other 'pages' on your WordPress site will use a
 * different template.
 *
 * Please see /external/starkers-utilities.php for info on Starkers_Utilities::get_template_parts()
 *
 * @package 	WordPress
 * @subpackage 	Starkers
 * @since 		Starkers 4.0
 * Template Name: Homepage
 */
?>
<?php Starkers_Utilities::get_template_parts( array( 'parts/shared/html-header', 'parts/shared/header' ) ); ?>

<?php if ( have_posts() ) while ( have_posts() ) : the_post(); ?>
<?php the_content(); ?>
<?php endwhile; ?>

<div id="global-container">

  <?php if (get_field('its_over_bar')) { ?>
                  <div class="its-over">
                    <?php the_field('its_over_bar');?>

                  </div>
                <?php } ?>
    
      <!-- START-PAGE -->
      <div id="start-page">

        <div class="header-bar" id="header-bar">
          <div class="content">
            <a href="#home"><img class="logo-small" src="<?php echo get_stylesheet_directory_uri(); ?>/images/logo-small.png" width="98" height="30" alt="Mind the Product" /></a>

            <div id="fb-root"></div>

            <script>(function(d, s, id) {
              var js, fjs = d.getElementsByTagName(s)[0];
              if (d.getElementById(id)) return;
              js = d.createElement(s); js.id = id;
              js.src = "//connect.facebook.net/en_US/all.js#xfbml=1&appId=100562470037834";
              fjs.parentNode.insertBefore(js, fjs);
            }(document, 'script', 'facebook-jssdk'));</script>
            
            <div class="fb-like" data-href="http://www.facebook.com/MindTheProduct" data-send="false" data-layout="button_count" data-width="100" data-show-faces="false"></div>

            <a href="https://twitter.com/share" class="twitter-share-button" data-url="http://conference.mindtheproduct.com" data-text="Excited to announce the conference of the year - Mind the Product 2012! Join us on September 28th!" data-via="mindtheproduct">Tweet</a>
            <script>!function(d,s,id){var js,fjs=d.getElementsByTagName(s)[0];if(!d.getElementById(id)){js=d.createElement(s);js.id=id;js.src="//platform.twitter.com/widgets.js";fjs.parentNode.insertBefore(js,fjs);}}(document,"script","twitter-wjs");</script>
          
            <!-- Place this tag where you want the +1 button to render -->
            <div style="float:right;padding:4px 0px 0px 0px;">
              <g:plusone size="medium"></g:plusone>
            </div

            <!-- Place this render call where appropriate -->
            <script type="text/javascript">
              (function() {
                var po = document.createElement('script'); po.type = 'text/javascript'; po.async = true;
                po.src = 'https://apis.google.com/js/plusone.js';
                var s = document.getElementsByTagName('script')[0]; s.parentNode.insertBefore(po, s);
              })();
            </script>

            <p class="date">Friday, 28th September 2012</p>
            
            <div class="buy-tickets">
              <a class="tab" href="http://mindtheproduct.ticketleap.com/mind-the-product-2012/" target="_blank">Buy Tickets</a>
            </div><!-- end buy-tickets -->
            
            <div class="clear"></div>
          </div><!-- end content -->
        </div><!-- end header-bar -->
      
        <div class="header" id="home">
          <h1><a href="#" class="logo">Mind the Product 2012 | Product Management Conference</a></h1>
          <div class="buy-tickets">
            <a class="tab" href="http://mindtheproduct.ticketleap.com/mind-the-product-2012/" target="_blank">Buy Tickets</a>
            <div class="price"><a href="http://mindtheproduct.ticketleap.com/mind-the-product-2012/" target="_blank">&pound;199 / ticket</a></div>
              </div><!-- end buy-tickets -->
          <div class="clear"></div>
        </div><!-- end header -->
        
        <div class="content">
          <ul class="main-nav">
            <li><a class="selected scroll" href="#home">Home</a></li>
            <li><a href="#speakers" class="scroll">Speakers</a></li>
            <li><a href="#schedule" class="scroll">Schedule</a></li>
            <li><a href="#location" class="scroll">Location</a></li>
            <li><a href="#the-team" class="scroll">The Team</a></li>
          </ul>
          
          <div class="indent">
            <h2>The business of building great products</h2>
            <p class="dark">On Friday, September 28th 2012<br /> at the Mermaid Conference Center in London</p>
            <p class="intro">MindtheProduct 2012 is the first conference of its kind - bringing together product management leaders from around the world to explore the intersection of design, technology and business - the business of building great products.</p>
            <div class="sponsors-small">
              <p>Platinum Sponsors</p>
              <a href="http://www.guardian.co.uk" title="The Guardian"><img src="<?php echo get_stylesheet_directory_uri(); ?>/images/the-guardian.png" width="230" height="41" alt="The Guardian" /></a>
              <br>
              <a href="http://www.ebay.co.uk" title="eBay"><img src="<?php echo get_stylesheet_directory_uri(); ?>/images/ebay-small.png" width="230" height="52" alt="eBay" /></a>
            </div><!-- end sponsors-small -->
            <div class="mediapartner">
              <p>Media Partner</p>
              <a href="http://gigaom.com" title="GigaOM"><img src="<?php echo get_stylesheet_directory_uri(); ?>/images/gigaom.png" width="180" height="28" alt="GigaOM" /></a>
            </div><!-- end sponsors-small -->
          </div><!-- end indent -->
        </div><!-- end content -->
        
        <!-- left background circles -->
        <div class="lbc-1"></div>
        <div class="lwc-1"></div>
        
        <!-- right background circles -->
        <div class="rwc-1"></div>
        <div class="rbc-1"></div>
        <div class="rwc-2"></div>
        <img class="conference-image" src="<?php echo get_stylesheet_directory_uri(); ?>/images/conference.png" width="290" height="439" alt="MindtheProduct conference" />

        <!-- ticket image -->
        <div class="ticket"></div>
      </div>
      <!-- END START-PAGE -->
      




      <!-- SPEAKERS -->
      <div id="speakers">
        
        <div class="top-bar">
          <div class="content">
            <h3>Speakers</h3>
            <div class="clear"></div>
          </div><!-- end content -->
        </div><!-- end top-bar -->
        
        <div class="content">
          <ul class="main-nav">
            <li><a href="#home" class="scroll">Home</a></li>
            <li><a class="selected scroll" href="#speakers">Speakers</a></li>
            <li><a href="#schedule" class="scroll">Schedule</a></li>
            <li><a href="#location" class="scroll">Location</a></li>
            <li><a href="#the-team" class="scroll">The Team</a></li>
          </ul><!-- end main-nav -->
          
          <div class="first speaker-right">
            <div class="white-bg">
              <img src="<?php echo get_stylesheet_directory_uri(); ?>/images/tom_hulme.png" width="152" height="152" alt="Marty Cagan" />
            </div>
            <h4>Tom Hulme</h4>
            <p class="speciality">Design Director, IDEO<br>Founder, OpenIDEO</p>
            <p class="info">Serial entrepreneur and investor.</p>
            <a class="twitter" href="http://twitter.com/thulme">thulme</a>
          </div><!-- end first speaker-right -->
          
          <div class="first speaker-left">
            <div class="white-bg">
              <img src="<?php echo get_stylesheet_directory_uri(); ?>/images/marty_cagan.png" width="152" height="152" alt="Alex Rainert" />
            </div>
            <h4>Marty Cagan</h4>
            <p class="speciality">Managing Partner, SVPG</p>
            <p class="info">Former VP of Product at eBay, AOL and Netscape.</p>
            <a class="twitter" href="http://twitter.com/cagan">cagan</a>
          </div><!-- end first speaker-left -->
          
          <div class="second speaker-right">
            <div class="white-bg">
              <img src="<?php echo get_stylesheet_directory_uri(); ?>/images/tom_loosemore.png" width="152" height="152" alt="Tom Loosemore" />
            </div>
            <h4>Tom Loosemore</h4>
            <p class="speciality">Deputy Director, GDS</p>
            <p class="info">Building gov.uk - a user centred single domain website for the UK Government.</p>
            <a class="twitter" href="http://twitter.com/tomskitomski">tomskitomski</a>
          </div><!-- end second speaker-right -->
          
          <div class="second speaker-left">
            <div class="white-bg">
              <img src="<?php echo get_stylesheet_directory_uri(); ?>/images/john_earner.png" width="152" height="152" alt="Tom Hulme" />
            </div>
            <h4>John Earner</h4>
            <p class="speciality">Entrepreneur in Residence, Accel</p>
            <p class="info">Former VP of Product at Playfish &amp; GM Playfish Studios at EA.</p>
            <a class="twitter" href="http://twitter.com/jearner">jearner</a>
          </div><!-- end second speaker-left -->
          
          <div class="third speaker-right">
            <div class="white-bg">
              <img src="<?php echo get_stylesheet_directory_uri(); ?>/images/matt_and_han.png" width="152" height="152" alt="TBA" />
            </div>
            <h4>Matthew Ogle<br>&amp; Hannah Donovan</h4>
            <p class="speciality">This Is My Jam</p>
            <p class="info">The product and design team behind This Is My Jam and Last.fm</p>
            <a class="twitter" href="http://twitter.com/flaneur">flaneur</a> <a class="twitter" href="http://twitter.com/han">han</a>
          </div><!-- end third speaker-right -->

          <div class="third speaker-left">
            <div class="white-bg">
              <img src="<?php echo get_stylesheet_directory_uri(); ?>/images/tom_chi.png" width="152" height="152" alt="Tom Chi" />
            </div>
            <h4>Tom Chi</h4>
            <p class="speciality">Experience Lead, Google X</p>
            <p class="info">Leading an experience team on projects from Google Glass to self-driving cars.</p>
            <a class="twitter" href="http://twitter.com/thegoodtomchi">thegoodtomchi</a>
          </div> <!-- end third speaker-left -->

          <div class="fourth speaker-right">
            <div class="white-bg">
              <img src="<?php echo get_stylesheet_directory_uri(); ?>/images/charles_adler.png" width="152" height="152" alt="Charles Adler" />
            </div>
            <h4>Charles Adler</h4>
            <p class="speciality">Co-founder &amp; Creative Director, Kickstarter</p>
            <p class="info">Designer and entrepreneur, leading the user experience and creative teams at Kickstarter.</p>
            <a class="twitter" href="http://twitter.com/cadler">cadler</a>
          </div> <!-- end fourth speaker-right -->

          <div class="fourth speaker-left">
            <div class="white-bg">
              <img src="<?php echo get_stylesheet_directory_uri(); ?>/images/tom_coates.png" width="152" height="152" alt="Tom Coates" />
            </div>
            <h4>Tom Coates</h4>
            <p class="speciality">Founder, Product Club</p>
            <p class="info">Former Head of Product at Yahoo Brickhouse, Fire Eagle, and BBC.</p>
            <a class="twitter" href="http://twitter.com/tomcoates">tomcoates</a>
          </div> <!-- end fourth speaker-right -->

                  <!-- panelists -->
          
          <div class="panelists">
           <h3>Panelists</h3>
           <div class="panel-member">
             <div class="white-bg">
              <img src="<?php echo get_stylesheet_directory_uri(); ?>/images/nic_newman.png" width="127" height="127" alt="Nic Newman" />
            </div>
            <h4>Nic Newman</h4>
            <p class="speciality">Visiting Fellow, Reuters Institute of Journalism</p>
           </div><!-- end panel-member -->
           
           <div class="panel-member">
             <div class="white-bg">
              <img src="<?php echo get_stylesheet_directory_uri(); ?>/images/tanya_cordrey.png" width="127" height="127" alt="Tanya Cordrey" />
            </div>
            <h4>Tanya Cordrey</h4>
            <p class="speciality">Digital Development Director, Guardian</p>
           </div><!-- end panel-member -->
           
           <div class="panel-member">
             <div class="white-bg">
              <img src="images/diana_stepner.png" width="127" height="127" alt="Diana Stepner" />
            </div>
            <h4>Diana Stepner</h4>
            <p class="speciality">Head of Future Technologies, Pearson</p>
           </div><!-- end panel-member -->
           


           
           <!--

           <div class="panel-member">
             <div class="white-bg">
              <img src="images/thumby.png" width="127" height="127" alt="Tom Hulme" />
            </div>
            <h4>Nic Newman</h4>
            <p class="speciality">Visiting Fellow, Reuters Institute of Journalism</p>
           </div>--><!-- end panel-member -->
          </div><!-- end panelists-->   

          <div class="clear"></div>
          
    
        </div><!-- end content -->
        
        <!-- left background circles -->
        <div class="lwc-2"></div>
        <div class="lbc-2"></div>
        
        <!-- right background circles -->
        <div class="rwc-3"></div>
      </div>
      <!-- END SPEAKERS -->
      
      <!-- SCHEDULE -->
      <div id="schedule">
        
        <div class="top-bar">
          <div class="content">
            <h3>Schedule</h3>
            <div class="clear"></div>
          </div><!-- end content -->
        </div><!-- end top-bar -->
        
        <div class="content">
          <ul class="main-nav">
            <li><a href="#home" class="scroll">Home</a></li>
            <li><a href="#speakers" class="scroll">Speakers</a></li>
            <li><a class="selected scroll" href="#schedule">Schedule</a></li>
            <li><a href="#location" class="scroll">Location</a></li>
            <li><a href="#the-team" class="scroll">The Team</a></li>
          </ul><!-- end main-nav -->
          
          <div class="become-a-sponsor">
           <h3>Interested in sponsoring?</h3>

            <h4>Download our information pack</h4>
            <a class="download" href="http://conference.mindtheproduct.com/sponsorpack.pdf" title"MindtheProduct Conference Sponsor Pack">Download</a>
          </div><!-- end become-a-sponsor-->
          
          <table>
            <tr>
              <td>09.00</td>
              <td><i>Registration</i></td>
            </tr>
            <tr>
              <td>10.00</td>
              <td>Intro</td>
            </tr>
            <tr>
              <td>10.15</td>
              <td>Marty Cagan</td>
            </tr>
            <tr>
              <td>11.00</td>
              <td>Tom Chi</td>
            </tr>
            <tr>
              <td>11.30</td>
              <td><i>Break</i></td>
            </tr>
            <tr>
              <td>12.00</td>
              <td>Tom Hulme</td>
            </tr>
            <tr>
              <td>12.30</td>
              <td>John Earner</td>
            </tr>
            <tr>
              <td>13.00</td>
              <td><i>Lunch (provided)</i></td>
            </tr>
            <tr>
              <td>14.30</td>
              <td>Charles Adler</td>
            </tr>
            <tr>
              <td>15.00</td>
              <td>Matt &amp; Hannah</td>
            </tr>
            <tr>
              <td>15.30</td>
              <td>Tom Loosemore</td>
            </tr>
            <tr>
              <td>16.00</td>
              <td><i>Break</i></td>
            </tr>
            <tr>
              <td>16.30</td>
              <td>Tom Coates</td>
            </tr>
            <tr>
              <td>17.00</td>
              <td>Panel - What's next for Product?</td>
            </tr>
            <tr>
              <td>17.45</td>
              <td>Closing remarks</td>
            </tr>
            <tr>
              <td class="space">18.00</td>
              <td class="space">After party!</td>
            </tr>
          </table>
          
          <div class="clear"></div>
        </div><!-- end content -->
        
        <!-- left background circles -->
        <div class="double-circle-1"></div>
        <div class="lbc-3"></div>
        
        <!-- right background circles -->
        <div class="rwc-4"></div>
        <div class="sponsors-circle">
          

          <div class="sponsors-large">
            <p>Sponsored by</p>
            <div class="sponsor-large"><img src="<?php echo get_stylesheet_directory_uri(); ?>/images/the-guardian-large.png" width="310" height="55" alt="The Guardian" /></div>
            <div class="sponsor-large space"><img src="<?php echo get_stylesheet_directory_uri(); ?>/images/ebay.png" width="310" height="70" alt="eBay" /></div>
            <div class="sponsor-medium"><img src="<?php echo get_stylesheet_directory_uri(); ?>/images/ticketmaster.png" width="142" height="50" alt="ticketmaster" /></div>
            <div class="sponsor-medium right"><img src="<?php echo get_stylesheet_directory_uri(); ?>/images/skyscanner.png" width="142" height="50" alt="Skyscanner" /></div>
            <div class="sponsor-medium"><img src="<?php echo get_stylesheet_directory_uri(); ?>/images/pearson.png" width="142" height="50" alt="Pearson" /></div>
            <div class="sponsor-medium right"><img src="<?php echo get_stylesheet_directory_uri(); ?>/images/skype.png" width="142" height="50" alt="Skype" /></div>
            <div class="sponsor-medium space right"><img src="<?php echo get_stylesheet_directory_uri(); ?>/images/product-focus.png" width="142" height="50" alt="Product Focus" /></div>
            <div class="sponsor-medium space"><img src="<?php echo get_stylesheet_directory_uri(); ?>/images/yammer.png" width="142" height="50" alt="Yammer" /></div>
            <div class="sponsor-small"><img src="<?php echo get_stylesheet_directory_uri(); ?>/images/d-labs.png" width="80" height="35" alt="D-Labs" /></div>
            <div class="sponsor-small middle"><img src="<?php echo get_stylesheet_directory_uri(); ?>/images/balsamiq.png" width="80" height="35" alt="Balsamiq" /></div>
            <div class="sponsor-small right"><img src="<?php echo get_stylesheet_directory_uri(); ?>/images/behance.png" width="80" height="35" alt="Behance" /></div>
            <div class="sponsor-small"><a href="http://www.prodpad.com/" title"ProdPad Product Management Tool"><img src="images/prodpad.png" width="80" height="35" alt="ProdPad" /></a></div>
            <div class="sponsor-small middle"><img src="i<?php echo get_stylesheet_directory_uri(); ?>/mages/blackblot.png" width="80" height="35" alt="Blackblot - Product Management Expertise" /></div>
            <div class="sponsor-small right"><img src="<?php echo get_stylesheet_directory_uri(); ?>/images/orrick.png" width="80" height="35" alt="Orrick LLP" /></div>
          </div>



          <!-- end sponsors-large -->
        </div><!-- end sponsors-circle -->
        <div class="rbc-2"></div>
        <div class="rwc-5"></div>
      </div>
      <!-- END SCHEDULE -->
        
      <!-- LOCATION -->
      <div id="location">
        
        <div class="top-bar">
          <div class="content">
            <h3>Location</h3>
            <div class="clear"></div>
          </div><!-- end content -->
        </div><!-- end top-bar -->
        
        <div class="content">
          <ul class="main-nav">
            <li><a href="#home" class="scroll">Home</a></li>
            <li><a href="#speakers" class="scroll">Speakers</a></li>
            <li><a href="#schedule" class="scroll">Schedule</a></li>
            <li><a class="selected scroll" href="#location">Location</a></li>
            <li><a href="#the-team" class="scroll">The Team</a></li>
          </ul><!-- end main-nav -->
        
          <div class="map">
            <img src="<?php echo get_stylesheet_directory_uri(); ?>/images/map.png" id="map-image" />
            <div class="clear"></div>
          </div><!-- end map -->
          
          <div class="map-overlay">
            <div class="pointer"></div>
              <div class="speech-bubble">
                <h4>Mermaid<br /> Conference Center</h4>
                <p class="address">Puddle Dock,<br /> London,<br /> Blackfriars<br /> EC4V 3DB</p>
                <a class="get-directions" href="http://maps.google.co.uk/maps?q=The+Mermaid+Conference+%26+Events+Centre,+London&hl=en&sll=53.800651,-4.064941&sspn=28.376106,48.779297&oq=The+Mermaid+C&hq=The+Mermaid+Conference+%26+Events+Centre,+London&t=m&z=15">Get directions</a>
              </div><!-- end speech-bubble -->
            </div><!-- end map-overlay -->
          
            <img src="<?php echo get_stylesheet_directory_uri(); ?>/images/theatre.png" id="theatre" />

          <div class="hotels">
            <h3>Where to stay</h3>
            <div class="hotel white">
              <h4><a href="http://www.grangehotels.com/Hotels-London/Grange-St-Pauls-Hotel/Grange-St-Pauls-Hotel.aspx">Grange St Pauls</a></h4>
              <p>★★★★★<br>
                10 Godliman Street<br>
                from £250/night</p>
            </div>

            <div class="hotel cyan">
              <h4><a href="http://www.ichotelsgroup.com/crowneplaza/hotels/gb/en/london/loncy/hoteldetail?cm_mmc=mdpr-_-googlemapsUK-_-cp-_-loncy">Crowne Plaza City</a></h4>
              <p>★★★★<br>
                19 New Bridge Street<br>
                from £130/night</p>
            </div>

            <div class="hotel blue">
              <h4><a href="http://www.accorhotels.com/gb/hotel-2814-mercure-london-bridge-hotel/index.shtml">Mercure Bankside</a></h4>
              <p>★★★★<br>
                71 Southwark Street<br>
                from £105/night</p>
            </div>
          </div>

          </div>

          <!-- end content -->
        
        <!-- left background circles -->
        <div class="lbc-4"></div>
        <div class="lwc-3"></div>
        
        <!-- right background circles -->
        <div class="rbc-3"></div>
      </div>
      <!-- END LOCATION -->
      
      <!-- THE TEAM -->
      <div id="the-team">
        
        <div class="top-bar">
          <div class="content">
            <h3>The team</h3>
            <div class="clear"></div>
          </div><!-- end content -->
        </div><!-- end top-bar -->
        
        <div class="content">
          <ul class="main-nav">
            <li><a href="#home" class="scroll">Home</a></li>
            <li><a href="#speakers" class="scroll">Speakers</a></li>
            <li><a href="#schedule" class="scroll">Schedule</a></li>
            <li><a href="#location" class="scroll">Location</a></li>
            <li><a class="selected scroll" href="#the-team">The Team</a></li>
          </ul><!-- end main-nav -->
          
          <div class="indent">
            <p class="intro">MindTheProduct 2012 is brought to you by a team of four product managers who spend all day building start-ups, and all night building a community  for other product managers.</p>
            <p><a href="http://twitter.com/bfgmartin">Martin</a> founded <a href="http://www.producttank.com">ProductTank</a>, a monthly product management meet up in London, which has been running for two years and now has over 900 members in the group and gets 120+ attendees at every meetup.</p>
            <p><a href="http://twitter.com/simplybastow">Janna</a> and <a href="http://twitter.com/simoncast">Simon</a> co-founded <a href="http://www.productcamplondon.com">ProductCamp London</a> bi-annually - a collaborative, user organized unconference based on the global ProductCamp.org.</p>
            <p>Together with <a href="http://twitter.com/smcllns">Sam</a> we got together and founded <a href="http://www.mindtheproduct.com">MindtheProduct.com</a>, a blog focused on product management, which we define as the intersection between design, technology and business. The blog combines our own editorial with community contributed posts.</p>
            <p>We are a not for profit company, funded by our generous sponsors and ticket sales, and driven by a passion for great products and the people who build them.</p>
            <p class="last">This site was designed by <a href="http://thisiscapra.com/"><img src="<?php echo get_stylesheet_directory_uri(); ?>/images/capra-logo.png" width="186" height="57" alt="Capra" /></a></p>
            
          </div><!-- end indent -->
        </div><!-- end content -->
        
        <!-- left background circles -->
        <div class="double-circle-2"></div>
        <div class="lbc-5"></div>
        
        <!-- right background circles -->
        <div class="blockquote-circle">
          <blockquote>One of the best ways to learn about Product Management from peers, and for product managers, 
          designers and marketers to socialise with like-minded people.</blockquote>
        </div><!-- end blockquote-circle -->
        <div class="rwc-6"></div>
        <div class="team-image"><img src="<?php echo get_stylesheet_directory_uri(); ?>/images/producttank_eric_ries.png" /></div>
      </div>
      <!-- END THE TEAM -->


<?php Starkers_Utilities::get_template_parts( array( 'parts/shared/footer','parts/shared/html-footer' ) ); ?>